...
function useRoot(){
  ...
  useEffect(()=>{
    try{
      ...
      if(raw){
        ...
        localStorage.setItem(BOARD_PREFIX+id, JSON.stringify(defaultBoard("Default Board")));
  ...
  const createBoard=name=>{
    ...
    setBoards(b=>[...b,{id,name}]); setActiveId(id);
  };
...
function useBoard(id){
  ...
  useEffect(()=>{ if(!id) return; const raw=localStorage.getItem(BOARD_PREFIX+id); if(raw) setData(JSON.parse(raw)); },[id]);
  useEffect(()=>{ if(!id||!data) return; localStorage.setItem(BOARD_PREFIX+id, JSON.stringify(data)); if(data.themeColor) document.documentElement.style.setProperty("--brand", data.themeColor); },[id,data]);
  return [data,setData];
}
...
function Setup({ root, board, setBoard }){
  ...
  const [authed,setAuthed] = useState(!lockEnabled);
  useEffect(()=>{
    if(lockEnabled){
  ...
  const exportJSON = () => {
    const a = Object.assign(document.createElement("a"), {
      href: URL.createObjectURL(new Blob([JSON.stringify(board,null,2)], {type:"application/json"})),
      download: `${(board.boardTitle||"board").replace(/\s+/g,"_")}.json`
    });
  ...
  const exportCSV = ()=>{
    const rows=[["Row","Col","Owner","Email","Paid","Note"]];
    ...
    const a = Object.assign(document.createElement("a"), {
      ...
      download: `${(board.boardTitle||"roster").replace(/\s+/g,"_")}.csv`
    });
    document.body.appendChild(a); a.click(); a.remove();
  };
  ...
  return html`
    <div>
      <div class="toolbar">
        <img src="./assets/logo-wordmark.png" alt="Wordmark" style="height:28px" onError=${e=>e.target.style.display="none"} />
      ...
      <div class="row">
        ...
        <div>
          ...
          <input readonly value=${location.origin + location.pathname + "?view=1"} onFocus=${e=>e.target.select()} onClick=${e=>e.target.select()}/>
        </div>
      </div>
...
function Board({ board, setBoard }){
  ...
  const winners = useMemo(()=>({
    q1: pick("q1"), q2: pick("q2"), q3: pick("q3"), q4: pick("q4")
  ...
  const computed = useMemo(()=>{
    ...
    if(board.payoutMode==="percent"){ const n=pot/100;
      return { q1:Math.round((p.q1||0)*n), q2:Math.round((p.q2||0)*n), q3:Math.round((p.q3||0)*n), q4:Math.round((p.q4||0)*n) };
    }
    return { q1:Math.round(p.q1||0), q2:Math.round(p.q2||0), q3:Math.round(p.q3||0), q4:Math.round(p.q4||0) };
  },[board.payoutMode,board.payouts,pot]);
  ...
  return html`
    <div>
      ...
      <div class="board-wrap">
        <div class="board">
          <div></div>
          ${board.topNums.map(n=> html`<div class="cell header-cell sticky-top" style="text-align:center">${board.teamTop} ${n}</div>`)}
          ${board.sideNums.map((sn,r)=> html`
            <div class="cell header-cell sticky-left">${board.teamSide} ${sn}</div>
          ...
          <div class="card">
            ...
            <div class="row" style="margin-top:6px">
              <div>
                ...
                <input type="number" value=${board.scores[q].top}  onInput=${e=> setBoard({...board, scores:{...board.scores, [q]:{...board.scores[q], top:e.target.value}}})}  ${VIEW_ONLY?"disabled":""}/>
              </div>
              <div>
                <label class="label">${board.teamSide}</label>
                <input type="number" value=${board.scores[q].side} onInput=${e=> setBoard({...board, scores:{...board.scores, [q]:{...board.scores[q], side:e.target.value}}})} ${VIEW_ONLY?"disabled":""}/>
              </div>
            </div>
...
function SquareEditor({ idx, board, setBoard, onClose }){
  ...
  const save = ()=>{ const next=[...board.squares]; next[idx]={owner,email,paid,note:customNote}; setBoard({...board,squares:next}); onClose&&onClose(); };
  const clear= ()=>{ const next=[...board.squares]; next[idx]={owner:"",email:"",paid:false,note:""}; setBoard({...board,squares:next}); onClose&&onClose(); };
    ...
    <div class="modal-backdrop" onClick=${onClose}>
      <div class="modal" onClick=${e=>e.stopPropagation()}>
        ...
        <div class="row" style="margin-top:8px">
          <div><label class="label">Buyer Name</label><input value=${owner} onInput=${e=>setOwner(e.target.value)}/></div>
          <div><label class="label">Email (optional)</label><input value=${email} onInput=${e=>setEmail(e.target.value)}/></div>
        </div>
...
/* ---------- mount controller (renders into #setup and #board) ---------- */
function App(){
  const root = useRoot();
  ...
  useEffect(()=>{
    try{
      ...
      if(raw){
        const j=JSON.parse(raw);
        const active=j.activeId || (j.boards?.[0]?.id);
        const bRaw = active ? localStorage.getItem(BOARD_PREFIX+active) : null;
        if(bRaw){
  ...
  useEffect(()=>{
    ...
    if(!root.activeId || !board){
      render(html`<div>Loading…</div>`, setupEl);
      render(html`<div></div>`, boardEl);
      return;
    }
    render(html`<${Setup} root=${root} board=${board} setBoard=${setBoard}/>`, setupEl);
    render(html`<${Board} board=${board} setBoard=${setBoard}/>`, boardEl);
  }, [root.activeId, board]);

  return null;
}
render(html`<${App}/>`, document.createElement("div"));
